...
func BuildAlertSnapshot(values map[string]*float64) (snapshot *AlertSnapshot, err error) {
	snapshot = &AlertSnapshot{Values: make(map[string]float64, len(values))}
	defer func() {
		if recovered := recover(); recovered != nil {
			err = fmt.Errorf("alert snapshot build failed: %v", recovered)
		}
	}()
